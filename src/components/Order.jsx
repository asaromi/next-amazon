import React, {useEffect} from 'react'
import Currency from './Currency'
import Image from 'next-image-export-optimizer'

const Order = ({ id, amount, amountShipping, images, items, timestamp, updatedAt }) => {
  return (
    <div className="relative border rounded-md">
      {/* top content */}
      <div className="flex items-center space-x-10 p-5 bg-gray-100 text-sm text-gray-600">
        <div>
          <p className="font-bold text-xs">ORDER PLACED</p>
          <p>{(timestamp || new Date()).toLocaleString()}</p>
        </div>

        <div>
          <p className="font-bold text-xs">Total</p>
          <p>
            <Currency currency="usd" price={amount}/> - Next Day Delivery{' '}
            <Currency currency="usd" price={amountShipping}/>
          </p>
        </div>

        <p className="text-sm whitespace-nowrap sm:text-xl self-end flex-1 text-right text-blue-500 font-medium">{items.length} items</p>
        <p className="absolute top-2 right-2 w-40 lg:w-72 truncate text-xs whitespace-nowrap">{id}</p>
      </div>

      <div className="p-5 sm:p-10">
        <div className="flex space-x-6 overflow-x-auto">
          {images.map((image, i) => (
            <Image
              key={i}
              alt={`image order ${i}`}
              className="h-20 object-contain sm:h-32"
              height={80}
              src={image}
              width={100}
            />
          ))}
        </div>
      </div>
    </div>
  )
}

export default Order
